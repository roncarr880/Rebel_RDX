...
PSK31:
 ...
 pin 4       Ra               |                             you can wire this on the top of the board
                              ...
                              --------/\/\/\/\------|
                                                   ...
                                                   ---
                                                  / / / 
 For the switch, I repurposed the on/off switch.  It enables the modulator and provides low power for WSPR, RTTY.
...
int serial_decode = 0;    // when enabled sends the decoded morse or rtty on the serial line 
                          ...
                          // this might interfer with CAT control but alowing both to be enabled at the same time.
...
int cut_num_enable = 0;   // cut_num_enable = 1 may be useful for when using the radio without a display
                          ...
                          // anyway long press of Function will announce the frequency even if this is not enabled. 
...
struct MEMORY {
  ...
  int sideband;
  int offset;
  int mode;     // WSPR not allowed as memory tune modes
};
...
#define MFSK_LOOPBACK 0      // software loopback for testing mfsk decoder algorithm - normally this should be zero
                             // greater than 1 is the percentage of errors introduced into the bit stream ( 4 bit errors at a time )
                             // I don't think the MFSK decoder works, but the transmit does.
/* ************************************************* */
...
const long Reference           = 50000352L;     //  was 49.99975e6
                                                ...
                                                //  this drifts a bit with temperature.
...
unsigned long  time_;          // ( mainly they are loop() variables )
...
long rc,tx;        // rc ? rx and tx look so alike with this font
   ...
   if( nokia == NO_DISPLAY || TUNE_LED_DISP ){  /* show tuning in LED's when display is off */
      ...
      set_display(sel,fun);
      if( nokia == NO_DISPLAY ) return;     /* don't need to run the nokia code in this case */
   }
   ...
   if( nokia == SPLIT_D ){    /* medium display */
      ...
      if( mem_tune ){         // first line has memory channel name 
         LCD.clrRow(0);
         LCD.print( memory[channel].  name_,0,0 );
      }
...
int but,sel,limit;
   ...
   /* do the action */
   if( function == 0 ){
     if( sel == LONGPRESS ){          // set DIGI mode, for no display or just a shortcut, function = BW, long press Select 
        digi_setup();
     }
...
//  ch2 = cval;
    ...
    pow_ = psk_lowpass(cval * cval , 2);    // agc after the loop arm filter for narrow band response
   if(  pow_ > 35000 ) agc -= 6;
   if(  pow_ < 25000 ) agc += 6;    // was 3
...
static int smax;
  ...
  if( indx == goalpost-2 )  decode_[0] = i+2, spot[3]= i;
  if( indx == goalpost-1 ) merge = 0x55,  decode_[1] = i+1, spot[4]= i;
  if( indx == goalpost )  decode_[2] = i, spot[5]= i;
  if( indx == goalpost+1 ) merge2 = 0x55,  decode_[3] = i+1, spot[6]= i;
  if( indx == goalpost+2 )  decode_[4] = i+2, spot[7]= i;
  ...
  if( ++indx > FFTSIZE/2 ){
     ...
     if(  decode_[2] >  decode_[0] &&  decode_[2] >  decode_[1] &&  decode_[2] >  decode_[3] &&  decode_[2] >  decode_[4] )
...
int second_decode_method(int  signal_ ){
...
static int farns,ch_count,fcount;
   ...
   if( (user[CODEREAD] == 0 && serial_decode == 0) || cread_indx < 2 || mode != CW ) return;
...
static int flip;                      // will work for DIGI also 
...
// mode_offset is zero for wspr but putting it into the freq calc in case that gets changed
...
long sample;
  ...
  // if( scope_active == 1 || scope_active == 2 ) scope();
   ...
   if( mode == RTTY || mode == PSK31 || mode == MFSK ) return timer + 10000;  
                                         // sample rate is 4000 for 62.5 hz per dft bucket - hopefully above highpass filter will be ok
...
void digi_setup(){
   ...
   Serial.end();
     // mode change so may need to stop some things from running
   if( mode == HELL ) detachCoreTimerService( feld_hell_core );
   if( mode == WSPR ) detachCoreTimerService( wspr_core );
   if( mode == DIGI ) detachCoreTimerService( digi_core );
...
struct MENU mode_menu = {
   8,
   "Select Mode",
   { "CW", "Hell", "PSK31", "RTTY", "MFSK16", "WSPR", "DIGI", "Mem Tune" }          
};
...
int i;
   ...
   if( SERIAL_MENUS ) Serial.begin(BAUDRATE);          // whatever baud you want.  I use 1200 as it is the ARGO baud rate
  ...
  //  }
   ...
   //  goalpost = 9;   
     ...
     if( mode == DIGI ) digi_setup();                 // can also be setup from long press of select
     ...
     else if( crh_tx_method == CRH_TX_PERL  ) serial_decode = 0;  // perl gui uses the serial port
 ...
 /*end of menus */
   ...
   top_active= 0;  
   if( serial_decode && cat_emu == NO_EMU ) Serial.begin(BAUDRATE); 
   if( cat_emu == K3_EMU ) Serial.begin(BAUDRATE);                              // 38400 is K3 baudrate?
   if( cat_emu == ARGO_EMU && mode != DIGI ) Serial.begin(BAUDRATE);            // DIGI already has Argo_emu serial running
...
static int counts;      // slow down the encoder 
   ...
   if( mode == WSPR ) mode = WEFAX;    // split modes not allowed in the memory
